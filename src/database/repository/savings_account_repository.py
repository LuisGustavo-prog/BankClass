...
async def login_saving(account_number: str, password: str):
    user = await savings_accounts.find_one({'account_number': account_number})
...
async def create_user_savings_accounts(cardholder_name: str, password: str):
    object_saving_account = SavingAccount(cardholder_name=cardholder_name, password=password)

    try:
        await savings_accounts.insert_one(object_saving_account.to_dict())
    except Exception as e:
        raise ValueError(f'Error saving user to database. {e}')

async def get_user(user_id: str):  
    try:
        user = await savings_accounts.find_one({'_id': user_id})
...
async def get_users(): 
    try:
        users = await savings_accounts.find().to_list(length=None)
...
async def update_user(user_id: str, password: str, new_cardholder_name: str = None, new_password: str = None):
    ...
    try:
        await savings_accounts.update_one(
            {'_id': user_id},
...
async def delete_user(user_id: str, password: str):
    ...
    try:
        await savings_accounts.delete_one({'_id': user_id})
    except Exception as e:
